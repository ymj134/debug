...
/*********************************************************************************
* Module       : top
* Style tag    : top_full_timing_stream_demo_v5_local_timing_offset
* Description  :
...
(
    ...
    // 本地 HDMI timing 相对源 timing 的固定延迟补偿（单位：pixel_clk 周期）
    // 当前默认 160，主要用于把板内回环链路的固定像素延迟补回来。
    // 若屏幕仍有轻微左右偏移，可在 144 / 152 / 160 / 168 / 176 之间微调。
    localparam integer HDMI_TIMING_DELAY = 160;
    ...
    // 本地 HDMI timing delay pipe
    reg [15:0] hdmi_h_cnt_d = 16'd0;
    reg [15:0] hdmi_v_cnt_d = 16'd0;
    reg        hdmi_vs_dly  = 1'b0;
    reg        hdmi_hs_dly  = 1'b0;
    reg        hdmi_de_dly  = 1'b0;

    reg [15:0] tp_h_cnt_pipe [0:HDMI_TIMING_DELAY-1];
    reg [15:0] tp_v_cnt_pipe [0:HDMI_TIMING_DELAY-1];
    reg        tp_vs_pipe    [0:HDMI_TIMING_DELAY-1];
    reg        tp_hs_pipe    [0:HDMI_TIMING_DELAY-1];
    reg        tp_de_pipe    [0:HDMI_TIMING_DELAY-1];
    ...
    assign link_up_rgb =
        (hdmi_de_dly && rx_stream_enable_pclk && rx_sym_valid && rx_sym_de) ? rx_sym_rgb
                                                                            : 24'h000000;
    ...
    //==========================================================================
    // 29) pixel 域：blink / 本地 timing delay / prefill / VS 锁流
    //==========================================================================
    always @(posedge pixel_clk or negedge pixel_rst_n) begin
        if (!pixel_rst_n) begin
            tp_vs_d               <= 1'b0;
            ...
            for (i_tpipe = 0; i_tpipe < HDMI_TIMING_DELAY; i_tpipe = i_tpipe + 1) begin
                tp_h_cnt_pipe[i_tpipe] <= 16'd0;
                tp_v_cnt_pipe[i_tpipe] <= 16'd0;
                tp_vs_pipe[i_tpipe]    <= 1'b0;
                tp_hs_pipe[i_tpipe]    <= 1'b0;
                tp_de_pipe[i_tpipe]    <= 1'b0;
            end
            ...
            // 本地 timing 固定延迟管线
            tp_h_cnt_pipe[0] <= tp_h_cnt;
            tp_v_cnt_pipe[0] <= tp_v_cnt;
            tp_vs_pipe[0]    <= tp_vs;
            tp_hs_pipe[0]    <= tp_hs;
            tp_de_pipe[0]    <= tp_de;

            for (i_tpipe = 1; i_tpipe < HDMI_TIMING_DELAY; i_tpipe = i_tpipe + 1) begin
                tp_h_cnt_pipe[i_tpipe] <= tp_h_cnt_pipe[i_tpipe-1];
                tp_v_cnt_pipe[i_tpipe] <= tp_v_cnt_pipe[i_tpipe-1];
                tp_vs_pipe[i_tpipe]    <= tp_vs_pipe[i_tpipe-1];
                tp_hs_pipe[i_tpipe]    <= tp_hs_pipe[i_tpipe-1];
                tp_de_pipe[i_tpipe]    <= tp_de_pipe[i_tpipe-1];
            end

            hdmi_h_cnt_d <= tp_h_cnt_pipe[HDMI_TIMING_DELAY-1];
            hdmi_v_cnt_d <= tp_v_cnt_pipe[HDMI_TIMING_DELAY-1];
            hdmi_vs_dly  <= tp_vs_pipe[HDMI_TIMING_DELAY-1];
            hdmi_hs_dly  <= tp_hs_pipe[HDMI_TIMING_DELAY-1];
            hdmi_de_dly  <= tp_de_pipe[HDMI_TIMING_DELAY-1];
